...
Italiano | English
This repository contains the software environment and teaching material for the Metabarcoding PhD Course 2026. The course introduces basic use of the Linux command line, processing of amplicon-sequencing data with AmpWrap, and downstream analysis in R/RStudio.
> [!IMPORTANT]
...
uname -m
```
Se `uname -m` restituisce `x86_64`, proseguire con la sezione Installazione comune su Ubuntu o WSL2.
---
Percorso B — Windows con WSL2 e Ubuntu
...
Cercare PowerShell.
Fare clic con il tasto destro su Windows PowerShell o Terminale.
Scegliere Esegui come amministratore.
Accettare la richiesta di autorizzazione di Windows.
Eseguire:
```powershell
...
un nome utente Linux, per esempio `mario`;
una password Linux;
...
Aprire l'ambiente
Aprire il terminale Ubuntu.
Attivare l'ambiente:
```bash
conda activate metabarcoding-phd
```
Spostarsi nella cartella di lavoro desiderata con `cd`.
...
uname -m
```
If `uname -m` returns `x86_64`, continue with Common installation on Ubuntu or WSL2.
---
Path B — Windows with WSL2 and Ubuntu
...
Accept the Windows permission request.
Run:
```powershell
wsl --install -d Ubuntu
```
...
a Linux user name, for example `mario`;
a Linux password;
...
conda activate metabarcoding-phd
```
Move to the required working directory with `cd`.
